...
export async function enhanceQuery(userQuery: string): Promise<string> {
  ...
  try {
    ...
    Your task is to improve the user's search query to make it more effective for semantic search in a vector database of Golem documentation.
...
function generateFallbackSummary(results: SearchResult[]): string {
  let summary = `# Golem Documentation Search Results\n\n`;
